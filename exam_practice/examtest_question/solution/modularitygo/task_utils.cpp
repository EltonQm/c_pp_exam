...
void print_command_error(                                   // Print error message
    ...
    for (const auto& arg : args) {                         // Loop through args
...
void print_invalid_filenames(                              // Print file warnings
    ...
    for (const auto& item : invalid_items) {               // Loop through items
        if (std::regex_search(item, dot_checker)) {        // Check for dot
            if (!has_obo_ext(item)) {                      // Invalid extension
                auto pos = item.rfind('.');                // Find last dot
                std::string ext = pos != std::string::npos ? item.substr(pos + 1) :
                    "(none)";                              // Get ext or none
            ...
            if (!fs::exists(item)) {                       // File does not exist
...
void print_invalid_namespace(                              // Print namespace warnings
    ...
    for (const auto& item : invalid_items) {               // Loop through items
        if (!std::regex_search(item, dot_checker_regex) && // No dot, not file
            validate_namespaces({item}).empty()) {         // Invalid namespace
            std::ostringstream oss;                        // Build error message
