...
#include "task2_utils.hpp"                              // Task utility declarations
...
int main(int argc, char *argv[]) {                     // Program entry point
    ...
    try {                                              // Try parsing args
        program.parse_args(argc, argv);                // Parse command-line args
    } catch (const std::exception& err) {              // Handle parse errors
    ...
    if (program.is_used("--consider-table") &&        // Check both flags used
        ...
        return 1;                                     // Exit with error
    } else if (program.is_used("--consider-table")) { // Check consider-table
        files_and_ns = program.get<std::vector<std::string>>("--consider-table");
        mode_consider = true;                         // Set consider-table mode
    } else if (program.is_used("--obsolete-stats")) { // Check obsolete-stats
    ...
    if (files_and_ns.empty()) {                      // Check for empty input
    ...
    for (const auto& item : valid_files) {           // Loop valid files
        std::cout << "Valid file - " << item << "\n"; // Print valid file
    }

    for (const auto& n : valid_namespaces) {         // Loop namespaces
        std::cout << "Valid namespace - " << n << "\n"; // Print valid namespace
    }

    std::cout << "Mode: " << (mode_consider ?        // Print selected mode
        "consider-table" : "obsolete-stats") << "\n";

    if (valid_files.empty()) {                       // Check no valid files
        print_command_usage(args, "No input files provided."); // Print error
        return 1;                                     // Exit with error
    }

    if (mode_consider) {                             // Check consider-table mode
        auto result = process_consider_table(valid_files, valid_namespaces);
        for (const auto& row : result) {             // Loop result pairs
            std::cout << row[0] << "\t" << row[1] << "\n"; // Print GO-ID pair
        }
    } else if (mode_obsolete) {                      // Check obsolete-stats mode
